...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $upload_method = $_POST['upload_method']; // নতুন: আপলোড মেথড
    ...
    } elseif ($upload_method === 'ftp' && !empty($_POST['ftp_file_select'])) {
        // FTP ফাইল থেকে পাথ তৈরি করা
        $file_path = 'uploads/files/ftp/' . basename($_POST['ftp_file_select']);
    }
    ...
    if ($is_edit) {
        ...
        if (empty($file_path)) {
            $response = ['success' => false, 'message' => 'App file is required. Please use direct upload or select a file from FTP.'];
